...
class ScoreParser:
    ...
    @staticmethod
    def is_bout_in_period(tags, action, minimal_bout_length):
        """
        Check if in the given range there is a bout of a specific behavior
    ...
    @staticmethod
    def is_last_seq_in_post_baseline_period(tags, action, minimal_bout_length):
        """
    ...
    @staticmethod
    def find_num_frames_from_last_bout(tags, action, minimal_bout_length):
        """
    ...
    @staticmethod
    def find_distances_from_last_bout(tags, action, minimal_time_for_bout=1):
        """
        function that check from a certain time point what is the distance from the last bout
        :param tags: range of behaviors prediction
        :param action: the behavior
            ...
            if tags[col] == action:
                ...
                if len(bout) > minimal_time_for_bout * second:
                    time_from_last_bout = ScoreParser.find_num_frames_from_last_bout(
                        ...
                        action=action, minimal_bout_length=1 * second)
    ...
    @staticmethod
    def find_bouts_long_short_np_wp(tags, action, short_long_threshold, threshold_for_bout_length=1):
        """
        ...
        for col in range(priors_period, len(tags) - window_end_to_bout_onset):
            if tags[col] == action:
                ...
                if len(bout) > threshold_for_bout_length * second:
                    ...
                    if not ScoreParser.is_bout_in_period(tags[bout[0] - priors_period:bout[0]], action,
                                                         minimal_bout_length=1 * second):
                        # np (Non Prior) - is the situation where we do not have a bout in the 10 sec before There is
